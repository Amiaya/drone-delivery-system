...
export const PAGINATION_FIELDS = ["limit", "offset", "nopaginate"];
export const ORDER_BY_FIELDS = ["order_by", "order"];
...
type RawPaginatedResults<T> = T & { row_count: string };
...
export class Repository<T> {
  ...
  protected async paginated(
    ...
    const raw: RawPaginatedResults<T>[] = await db
      .select(this.knex.raw("count(*) OVER() AS row_count"), "*")
      .limit(limit)
      .offset(offset);
    ...
    const total = parseInt(raw[0].row_count);
    raw.forEach(r => {
      delete r["row_count"];
    });
